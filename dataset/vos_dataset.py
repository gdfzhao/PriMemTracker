        ...
        for vid in vid_list:
            if subset is not None:
                if vid not in subset:
                    continue
            frames = sorted([f for f in os.listdir(os.path.join(self.im_root, vid)) if f[-4:] == '.jpg'])
    ...
    def __getitem__(self, idx):
        ...
        while trials < 5:
            ...
            for f_idx in frames_idx:
                ...
                ori_images.append(path.join(vid_im_path, jpg_name))
                this_im = self.all_im_dual_transform(this_im)
                this_im = self.all_im_lone_transform(this_im)
                reseed(sequence_seed)
                this_gt = Image.open(path.join(vid_gt_path, png_name)).convert('P')
                # print("JPG name:{}, max:{}".format(path.join(vid_im_path, jpg_name), np.array(this_gt).max()))
                ...
                # print("THIS gt shape", this_gt.shape)
                # print("THIS gt max", this_gt.max())
                images.append(this_im)
                masks.append(this_gt)

            # print("masks", masks[0].max())

            images = torch.stack(images, 0)  # [8, 3, 384, 384]
            np.set_printoptions(threshold=np.inf)
            # print(masks[0])
            # for i in range(len(masks)):
            #     print("mask {}, labels:{}".format(i, np.unique(masks[i])))
        ...
        cls_gt_one_hot = np.zeros((self.num_frames, self.max_num_obj, 384, 384), dtype=np.int64)
        first_frame_gt = np.zeros((1, self.max_num_obj, 384, 384), dtype=np.int64)
        mask_random = False
        for i, l in enumerate(target_objects):
            this_mask = (masks==l)
            if mask_random:
                this_mask = (this_mask * np.random.random(this_mask.shape)) > 0.5

            cls_gt[this_mask] = i+1
            cls_gt_one_hot[:, i, ...] = this_mask
            # print(cls_gt_one_hot.max())

            first_frame_gt[0,i] = (this_mask[0])

        # cls_gt_one_hot = np.expand_dims(cls_gt_one_hot, 1)
        # print(cls_gt_one_hot.shape)
        cls_gt = np.expand_dims(cls_gt, 1)
        # print(cls_gt.shape)

        # 1 if object exist, 0 otherwise
        selector = [1 if i < info['num_objects'] else 0 for i in range(self.max_num_obj)]
        # print(info['num_objects'])
        # print(selector)
        selector = torch.FloatTensor(selector)
        info['ori_images'] = ori_images

        data = {
            'rgb': images,
            'first_frame_gt': first_frame_gt,
            'cls_gt': cls_gt,
            'cls_gt_one_hot': cls_gt_one_hot,
            'selector': selector,
            'info': info,
        }
