...
from util.load_subset import load_sub_davis, load_sub_yv, load_sub_ovis
...
for si, stage in enumerate(stages_to_perform):
    ...
    def construct_loader(dataset):
        ...
        train_loader = DataLoader(dataset, config['batch_size'], sampler=train_sampler, num_workers=config['num_workers'],
                                worker_init_fn=worker_init_fn, drop_last=True, shuffle=False)
    ...
    def renew_vos_loader(max_skip, finetune=False):
        ...
        ovis_dataset = VOSDataset(path.join(ovis_root, 'JPEGImages'),
                                  path.join(ovis_root, 'Annotations'), max_skip // 5, is_bl=False,
                                  subset=load_sub_ovis(), num_frames=config['num_frames'], finetune=finetune)
        # train_dataset = ConcatDataset([davis_dataset]*5 + [yv_dataset])
        train_dataset = ConcatDataset([davis_dataset]*5 + [yv_dataset] + [ovis_dataset])
    ...
    def renew_ovis_loader(max_skip, finetune=False):
        # //5 because we only have annotation for every five frames
        train_dataset = VOSDataset(path.join(ovis_root, 'JPEGImages'),
                            path.join(ovis_root, 'Annotations'), max_skip // 5, is_bl=False, subset=load_sub_ovis(), num_frames=config['num_frames'], finetune=finetune)


        print(f'OVIS dataset size: {len(train_dataset)}')
        print(f'Renewed with {max_skip=}')
    ...
    Determine max epoch
    """
    # print("config['iterations']:", config['iterations'])
