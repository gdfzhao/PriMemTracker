...
class FeatureFusionBlock(nn.Module):
    ...
    def forward(self, x, g):
        ...
        g = self.distributor(x, g) # [8, 3, 1280(1024+256), 24, 24])
        # import ipdb
        # ipdb.set_trace()
        g = self.block1(g)
...
class ValueEncoder(nn.Module):
    def __init__(self, value_dim, hidden_dim, single_object=False, expand_mask=False, is_train=False):
        super().__init__()
        
        self.single_object = single_object
        if expand_mask:
            network = resnet.resnet18(pretrained=True, extra_dim=2 if single_object else 3)
        else:
    ...
    def forward(self, image, image_feat_f16, h, masks, others, is_deep_update=True, cls_gt=None):
        # image_feat_f16 is the feature from the key encoder
        # TODO: Concat this frame mask
        # Encode value only in one frame
        # import pdb
        # pdb.set_trace()
        if not self.single_object:
            # mask 三个1 torch.Size([8, 3, 384, 384]) others 一样
            # import pdb
            # pdb.set_trace()
            # bs = masks.shape[0]
            ...
            # this_frame_gt = np.zeros((bs, num_objects, 384, 384), dtype=np.int64)

            # if cls_gt is not None: # [8, 1, 384, 384] max = num_objects
            #
            #     for i in range(num_objects):
            #         this_mask = (cls_gt == i + 1)
            #
            #         this_frame_gt[:, i, this_mask] = 1
            ...
            if self.expand_mask:
                ...
                if self._is_train:
                    ...
                    g = torch.stack([masks, others, cls_gt], 2)         # torch.Size([8, 3, 2, 384, 384])
                else:
                    g = torch.stack([masks, others, masks], 2)         # torch.Size([8, 3, 2, 384, 384])
            else:
                g = torch.stack([masks, others], 2)         # torch.Size([8, 3, 2, 384, 384])

        else:
            g = masks.unsqueeze(2)
        g = self.distributor(image, g)   # torch.Size([8, 3, 5, 384, 384]) = rgb + masks(first frame) + others
                                         # = concat(image[8, num_object, 3, 384, 384], g[8, 3, 2, 384, 384])
...
class KeyEncoder_With_IS(nn.Module):
    def __init__(self):
        super().__init__()
        network = resnet.resnet50(pretrained=True)
        self.conv1 = network.conv1
    ...
    def forward(self, f):
        ...
        return {'res5': f32, 'res4': f16, 'res3': f8, 'res2': f4}
